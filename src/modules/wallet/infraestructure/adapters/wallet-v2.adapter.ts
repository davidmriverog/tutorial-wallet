...
@Injectable()
export class Wallet2Adapter implements WalletRepositoryPort {
  constructor(private readonly walletMapper: WalletMapper) {}

  async balance(walletId: any): Promise<WalletModel> {
    console.log(
      `se conecta a la wallet orlando y muestra el balance con el id ${walletId}`,
    );
  ...
  async credit(walletId: any, amount: number): Promise<boolean> {
    console.log(
      `se conecta a la wallet orlando y acredita con el id ${walletId} monto ${amount}`,
    );
  ...
  async debit(walletId: any, amount: number): Promise<boolean> {
    console.log(
      `se conecta a la wallet orlando y debita con el id ${walletId} monto ${amount}`,
    );
